...
- **2FA-required failures on session commands now report `E_2FA_REQUIRED` (exit 9), not `E_AUTH` (exit 4).** The per-item/session error classifier ignored an `APIError`'s explicit `Code`, so a missing-OTP failure on `query`/`dict`/etc. collapsed to the status-derived `E_AUTH` — telling an agent to fix credentials instead of supplying `--otp`. It now honors the explicit code, matching `handleAPIError` (CLI-SPEC §6, §16.3).
...
- Install-method detection no longer classifies a standalone binary as pip-managed just because its path contains the substring `pip` or `conda`. Only genuine pip/venv layouts (a `site-packages`, `venv`/`.venv`, or `lib`+`python` path segment) are treated as pip, so `update` cannot misroute into a bogus `pip install` for a binary that merely lives under a directory whose name happens to contain those letters.
...
- **`update` failures are now classified onto the error taxonomy instead of collapsing into `E_NETWORK`/hardcoded exits.** Concretely:
  ...
  - `discover`/`download` HTTP failures are split by upstream status — `403 → E_FORBIDDEN`, `404 → E_NOT_FOUND` (non-retryable), `408 → E_TIMEOUT`, `429 → E_RATE_LIMITED`, `5xx → E_SERVER` — instead of every transport failure looking like a transient `E_NETWORK`.
